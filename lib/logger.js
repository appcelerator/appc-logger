...
function createRestifyLogger (server, options) {
	...
	server.on(afterEvent,function(req, res){
		...
		req.log.info({req_id:req.getId(), body: req.body, ignore:true});
		var result = {res:res, req_id: req.getId(), req:req, name: req.log.name, logname: req.log.logname, duration: duration};
		requestLogger.info(result);
		fs.writeFile(req.log.logname+'.metadata',JSON.stringify(result));
	});
