...
function createHttpLogger(server, serviceType, options) {
	...
	// only create the request logger if we have a log directory to log to
		...
		adiLogger = !skipRequestLog && fs.existsSync(logDir) && bunyan.createLogger({
			...
			streams: [{
				...
				count: 1,
				level: 'trace',
				path: path.join(logDir, options && options.requestsLogFilename || 'adiAnalytics.json')
			}]
		})
	...
	function logRequest(req, res) {
		...
		if (shouldLogRequest && req.log && req.logname && (req.logmetadata === undefined || req.logmetadata)) {
			...
			adiLogger.info({
				...
				correlationId: req._logname.substring(req._logname.indexOf('request-') + 8),
				legs: [
					{
						uri: req.route.path,
						...
						bytesSent: null,
						bytesReceived: null,
						remoteName: req.remoteAddress,
						remoteAddr: req.remoteAddress,
						localAddr: req.remoteAddress,
