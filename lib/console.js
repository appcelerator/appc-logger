...
function checkColorize() {
	...
	for (let c = 1; c < process.argv.length; c++) {
...
ConsoleLogger.prototype.write = function (record) {
	...
	const level = this.LevelMapping[record.level],
		color = this.colorize;
	let args = [];
	...
	if (!msg || msg === ' ') {
		...
		const fields = _.omit(record, 'name', 'prefix', 'showtab', 'showcr', 'hostname', 'pid', 'level', 'msg', 'time', 'v', 'problemLogName');
		if (_.isEmpty(fields)) {
			msg = '';
		} else {
